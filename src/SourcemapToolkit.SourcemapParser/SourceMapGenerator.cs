...
namespace SourcemapToolkit.SourcemapParser
{
    ...
    internal class MappingGenerateState
    {
        ...
        /// </summary>
        public readonly SourcePosition LastGeneratedPosition = new SourcePosition(0, 0);

        /// <summary>
        /// Last location of the code in the source code
        /// </summary>
        public readonly SourcePosition LastOriginalPosition = new SourcePosition(0, 0);
    ...
    public class SourceMapGenerator
    {
        ...
        /// </summary>
        public string GenerateSourceMapInlineComment(SourceMap sourceMap,
        ...
        internal void SerializeMappingEntry(MappingEntry entry, MappingGenerateState state, StringBuilder output)
        {
            if (state.LastGeneratedPosition.ZeroBasedLineNumber > entry.GeneratedSourcePosition.ZeroBasedLineNumber)
            {
                throw new InvalidOperationException(
                    $"Invalid sourmap detected. Please check the line {entry.GeneratedSourcePosition.ZeroBasedLineNumber}");
            }
            ...
            Base64VlqEncoder.Encode(output,
                entry.GeneratedSourcePosition.ZeroBasedColumnNumber -
                state.LastGeneratedPosition.ZeroBasedColumnNumber);
            ...
            if (entry.OriginalFileName != null)
            {
                ...
                if (sourceIndex < 0)
                {
                    throw new SerializationException(
                        "Source map contains original source that cannot be found in provided sources array");
                }
                ...
                Base64VlqEncoder.Encode(output,
                    entry.OriginalSourcePosition.ZeroBasedLineNumber - state.LastOriginalPosition.ZeroBasedLineNumber);
                state.LastOriginalPosition.ZeroBasedLineNumber = entry.OriginalSourcePosition.ZeroBasedLineNumber;

                Base64VlqEncoder.Encode(output,
                    entry.OriginalSourcePosition.ZeroBasedColumnNumber -
                    state.LastOriginalPosition.ZeroBasedColumnNumber);
                ...
                if (entry.OriginalName != null)
                {
                    ...
                    {
                        throw new SerializationException(
                            "Source map contains original name that cannot be found in provided names array");
                    }
